...
As shown here, the curve for Random Forest is higher than all other models for nearly all thresholds, and this becomes even more apparent in the bar chart below. Even at its worst, random forest performs better than the next-best classifier of XGBoost.
...
Although Random Forest can make good predictions, it is also an uninterpretable [black box](https://en.wikipedia.org/wiki/Black_box). Even if we could zoom in on every single decision that went into the prediction, it would be impossible to interpret.
